...
mm_free_blocks(block_meta_data_t *to_be_free_block) {
    ...
    if(next_block) {
        /* Scenario 1 : When data block to be free is not the last one*/
        ...
        char *end_address_of_free_data_block = (char *)((char *)(to_be_free_block + 1) 
                                        + to_be_free_block->block_size);
        int internal_mem_fragmenation = (int)((unsigned long)end_address_of_vm_page -
                    (unsigned long)end_address_of_free_data_block);
        to_be_free_block->block_size += internal_mem_fragmenation;
    }
    ...
    mm_add_free_block_meta_data_to_free_block_list(hosting_page->page_family, return_block);
